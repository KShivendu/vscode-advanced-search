...
export async function structuredSearch(
	...
	const searchCmd = `comby -exclude-dir ${excludeDir.join(
		',',
	)} ${JSON.stringify(
		query,
	)} '' -json-lines -matcher ${langauge} -match-only -d ${dir}`;
...
export async function structuredReplaceCommand(
	searchQuery: string,
	replaceQuery: string,
	dir: string,
	language: string,
): Promise<ReplaceResult[]> {
	const replaceCommand = `comby -exclude-dir  ${excludeDir.join(
		',',
	)} ${JSON.stringify(searchQuery)} ${JSON.stringify(
		replaceQuery,
	)} -json-lines -matcher ${language} -d ${dir}`;
				...
				.map((result: any) => {
					return {
						...
						updatedFileContent: result.rewritten_source
							.replace(/\\"/g, '"')
							.replace(/\\n/g, '\n'),
						replacements: result.in_place_substitutions.map(
							(replacement: any) => {
								return {
...
export async function structuredReplace(
	searchQuery: string,
	replaceQuery: string,
	dir: string,
	language: string,
): Promise<ReplaceResult[]> {
	const result = await structuredReplaceCommand(
		searchQuery,
		replaceQuery,
		dir,
		language,
	...
	for (const file of result) {
		...
		const finalContent = await new Promise((res, rej) => {
			vscode.workspace.onDidCloseTextDocument(closedDoc => {
				const tempWasClosed =
					closedDoc.fileName.endsWith('tempFile') && closedDoc.isClosed;
				if (tempWasClosed) {
					res(closedDoc.getText());
				}
			});
		});
